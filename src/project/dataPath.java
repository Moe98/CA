...
public class dataPath {
	...
	static boolean[][] ALU(boolean[] readData1, boolean[] readData2, boolean[] ALU_Control) throws Exception 
	{
		...
		if(control==0)//ADD
			...
			return new boolean[][] { toBooleanArray(Integer.bitCount(data1)), toBooleanArray(0) };//TODO check if it uses data1 or data2
		else if(control==6)//SWITCH_SIGN
			return new boolean[][] { toBooleanArray(-1*data1), toBooleanArray(0) };//TODO check if it uses data1 or data2
		else if(control==7)//POWER
			return new boolean[][] { toBooleanArray((int)Math.pow(data1, data2)), toBooleanArray(0) };
		else if(control==8)//ABS
			return new boolean[][] { toBooleanArray(Math.abs(data1)), toBooleanArray(0) };//TODO check if it uses data1 or data2
